...
namespace Slon.Benchmark
{
    ...
    public class Benchmarks
    {
        ...
        public async ValueTask SetupPipelines()
        {
            ...
            // Pipelining
            var dataSource = new Slon.SlonDataSource(Options with {PoolSize = 1}, ProtocolOptions);
            _conn = new Slon.SlonConnection(dataSource);
            await _conn.OpenAsync();
            _pipeliningCommand = new Slon.SlonCommand(_commandText, _conn);

            var dataSource2 = new Slon.SlonDataSource(Options with {PoolSize = 1}, ProtocolOptions);
            _multiplexedPipeliningCommand = dataSource2.CreateCommand(_commandText);

            // Multiplexing
            var dataSource3 = new Slon.SlonDataSource(Options, ProtocolOptions);
