...
static class SpanBufferExtensions
{
    // Copies whatever is committed to the given writer.
    public static void CopyTo<T, TOutput>(ref this SpanBufferWriter<T> buffer, ref SpanBufferWriter<TOutput> output) where T : ICopyableBuffer<byte>, IBufferWriter<byte> where TOutput : IBufferWriter<byte>
    {
        buffer.Output.CopyTo(output.Output);
        output.Advance((int)buffer.BytesCommitted);
    }

    public static void WriteRaw<T>(ref this SpanBufferWriter<T> buffer, ReadOnlySpan<byte> value) where T : IBufferWriter<byte>
    {
        buffer.Write(value);
    }

    public static void WriteUShort<T>(ref this SpanBufferWriter<T> buffer, ushort value)  where T : IBufferWriter<byte>
    {
        buffer.Ensure(sizeof(short));
        BinaryPrimitives.WriteUInt16BigEndian(buffer.Span, value);
        buffer.Advance(sizeof(short));
    ...
    public static void WriteShort<T>(ref this SpanBufferWriter<T> buffer, short value)  where T : IBufferWriter<byte>
    {
        buffer.Ensure(sizeof(short));
        BinaryPrimitives.WriteInt16BigEndian(buffer.Span, value);
        buffer.Advance(sizeof(short));
    ...
    public static void WriteInt<T>(ref this SpanBufferWriter<T> buffer, int value) where T : IBufferWriter<byte>
    {
        buffer.Ensure(sizeof(int));
        BinaryPrimitives.WriteInt32BigEndian(buffer.Span, value);
        buffer.Advance(sizeof(int));
    ...
    public static void WriteUInt<T>(ref this SpanBufferWriter<T> buffer, uint value) where T : IBufferWriter<byte>
    {
        buffer.Ensure(sizeof(uint));
        BinaryPrimitives.WriteUInt32BigEndian(buffer.Span, value);
        buffer.Advance(sizeof(uint));
    ...
    public static void WriteCString<T>(ref this SpanBufferWriter<T> buffer, string value) where T : IBufferWriter<byte>
    {
        buffer.WriteEncoded(value.AsSpan(), Encoding.UTF8);
        buffer.WriteByte(0);
    }

    public static void WriteByte<T>(ref this SpanBufferWriter<T> buffer, byte b)
        where T : IBufferWriter<byte>
    {
        buffer.Ensure(sizeof(byte));
        buffer.Span[0] = b;
    ...
    public static Encoder? WriteEncoded<T>(ref this SpanBufferWriter<T> buffer, ReadOnlySpan<char> data, Encoding encoding, Encoder? encoder = null)
        where T : IBufferWriter<byte>
    {
        if (data.IsEmpty)
            return null;
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    static Encoder? WriteEncodedMultiWrite<T>(ref this SpanBufferWriter<T> buffer, ReadOnlySpan<char> data, int encodedLength, Encoding encoding, Encoder? enc = null)
        where T : IBufferWriter<byte>
    {
        var source = data;
        var totalBytesUsed = 0;
